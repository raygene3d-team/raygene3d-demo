...
)
{
  ...
  const Meshlet meshlet = meshlet_buffer[mlet_offset + groupID];
  SetMeshOutputCounts(meshlet.vidx_count, meshlet.tidx_count);

  {
    for (uint i = 0; i < VERT_ITERATIONS; ++i)
    {
      ...
      {
        uint idx = v_index_buffer[vidx + meshlet.vidx_offset + vidx_offset];

        float3 opos = vertex_buffer[idx + vert_offset].pos;
        float3 wpos = opos; //(transform  * float4(opos, 1.0)).xyz;

        float d = vertex_buffer[idx + vert_offset].sgn;
        float3 nrm = vertex_buffer[idx + vert_offset].nrm;
        float u = vertex_buffer[idx + vert_offset].tc0.x;
        float3 tgn = vertex_buffer[idx + vert_offset].tgn;
        float v = vertex_buffer[idx + vert_offset].tc0.y;

        vert_out[vidx].w_pos_d = float4(wpos, d);
